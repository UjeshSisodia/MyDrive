...
exports.imageUpload = async(request, respond) => {
    try{
        ...
        let fileType = uploadingImage.name.split(".")[1];
        ...
        const uploadedImage = await uploadFiletoCloudinary(uploadingImage, "Ujesh");
...
exports.videoUpload = async(request, respond) => {
    try{
        ...
        const uploadedVideo = await uploadFiletoCloudinary(uploadingVideo, "Ujesh");
...
exports.imageSizeReducer = async(request, respond) => {
    try{
        ...
        const uploadedImage = await uploadFiletoCloudinary(uploadingImage, "Ujesh" , 30);
        const imageDatabaseEntry = await File.create({name,email,tags,imageUrl:uploadedImage.secure_url});
        console.log("File uploaded successfully to cloudinary");
        respond.status(200).json({
            success:true,
